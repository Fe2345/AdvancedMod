using AdvancedMod.MutationMode.Net;
using AdvancedMod.MutationMode.NPCMatching;
using AdvancedMod.MutationMode.Net.Strategies;
...
{
    public abstract class MutationModeNPCBehaviour
    {
        internal static Mod mod => AdvancedMod.Instance;

        public static List<MutationModeNPCBehaviour> AllMutationModeNpcBehaviours = new List<MutationModeNPCBehaviour>();

        public NPCMatcher Matcher;

        public void Register()
        {
            AllMutationModeNpcBehaviours.Add(this);
        }
        ...
        public int GetBytesNeeded()
        {
            ...
            foreach (CompoundStrategy strategy in netInfo.Values)
            {
                if (strategy.Equals(BoolStrategies.CompoundStrategy))
                    ...
                    AdvancedMod.Instance.Logger.Warn("didn't recognize strategy!");
            }
        ...
        /// </summary>
        public virtual MutationModeNPCBehaviour NewInstance() => (MutationModeNPCBehaviour)MemberwiseClone();
        ...
        protected static void NetSync(NPC npc, bool onlySendFromServer = true)
        {
            ...
            npc.GetGlobalNPC<NewMutationModeGlobalNPC>().NetSync(npc);
        }
        ...
        {
            return ModContent.Request<Texture2D>("AdvancedMod/NPCs/" + (recolor ? "Resprites/" : "Vanilla/") + texture, AssetRequestMode.ImmediateLoad);
        }
